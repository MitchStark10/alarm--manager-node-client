...
describe("Test the API client", () => {
  it("Should connect given correct connection details", () => {
    const client = new AMClient(
      getEnvVarString("AM_HOST"),
      getEnvVarString("EMAIL"),
      getEnvVarString("API_KEY")
    );
    return expect(
  ...
  it("Should throw an error if initialized with empty data", () => {
    try {
      ...
      console.error(
        "An AMClient initialized with empty data should throw an error."
      );
  ...
  it("Should throw an error if a request fails", () => {
    ...
    return expect(
      client.postNewAlert(
        "Invalid Host",
        "This alarm is expected to fail to be created",
        true
      )
    ).rejects.toEqual("Error occurred while adding alarm");
  });
});
